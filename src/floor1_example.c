...
/*
 * your own type definition with type extension.
 */
...
/** 
 * Fonction qui permet le décodage d'un floor de type 1 dans le flux Vorbis.
 * Cette fonction est static, mais les objets floor_t de type 1 pointeront
 * dessus (via leur champ decode).
 * 
 ...
 * + erreur read_nbits
 */
static status_t floor_type1_decode(vorbis_stream_t *stream, floor_t *floor_cfg,
				   sample_t *v, uint16_t v_size)
{

     	floor_type1_t *floor = (floor_type1_t *) floor_cfg;
...
/** 
 * Fonction qui libère la mémoire de la structure qui lui est fournie 
 * en argument, et qui doit etre un floor de type 1.
 * Cette fonction est static, mais les objets floor_t de type 1 pointeront
 * dessus (via leur champ free).
 * 
 * @param[in] floor_t est un pointeur sur le floor à libérer.
 */
static void floor_type1_free(floor_t *fl)
{
				...
				floor_t **floor, floor_data_t *data)
{
	...
	floor_type1_t *floor = calloc(1, sizeof(floor_type1_t));
	*pfloor = (floor_t *)floor;
...
status_t floor_type1_data_new(floor_data_t **pfl_data, uint16_t *blocksize)
{
     ...
     *pfl_data = malloc(sizeof(floor_data_t));
...
status_t floor_type1_data_allocate(floor_data_t *fl_data)
{
	...
	floor_type1_data_t *data = (floor_type1_data_t *) fl_data;

	/* to be completed */

	data->dummy = malloc(sizeof(int)*NB);
